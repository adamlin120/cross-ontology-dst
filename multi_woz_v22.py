...
_URL_LIST = [
    (
        "dialogue_acts",
        "https://github.com/budzianowski/multiwoz/raw/master/data/MultiWOZ_2.2/dialog_acts.json",
    )
]
...
_URL_LIST += [
    (
        "v2.2",
        "https://ytlin.s3-ap-northeast-1.amazonaws.com/data/schemas/2.2/schema.json",
    ),
    (
        "v2.2_active_only",
        "https://ytlin.s3-ap-northeast-1.amazonaws.com/data/schemas/2.2/schema.json",
    ...
    (
        "v2.2_slots_desc",
        "https://ytlin.s3-ap-northeast-1.amazonaws.com/data/schemas/2.2/schema_w_match-desc.json",
    ),
    (
        "v2.2_slots_onto",
        "https://ytlin.s3-ap-northeast-1.amazonaws.com/data/schemas/2.2/schema_w_match-ontology.json",
    ),
    (
        "v2.2_slots_both",
...
class MultiWozV22(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        datasets.BuilderConfig(
            name="v2.2", version=datasets.Version("2.2.0"), description="MultiWOZ v2.2"
        ),
        datasets.BuilderConfig(
                            ...
                            "slots": datasets.Sequence(
                                {
                                    ...
                                    "copy_from_value": datasets.Sequence(
                                        datasets.Value("string")
                                    ),
                                }
                            ),
                            ...
                            "frames": datasets.Sequence(
                                {
                                    ...
                                    "state": {
                                        ...
                                        "slots_values": datasets.Sequence(
                                            {
                                                "slots_values_name": datasets.Value(
                                                    "string"
                                                ),
                                                "slots_values_list": datasets.Sequence(
                                                    datasets.Value("string")
                                    ...
                                    "dialog_act": datasets.Sequence(
                                        {
                                            ...
                                            "act_slots": datasets.Sequence(
                                                datasets.Features(
                                                    {
                                                        "slot_name": datasets.Value(
                                                            "string"
                                                        ),
                                                        "slot_value": datasets.Value(
                                                            "string"
                                                        ),
                                                    }
                                                ),
                            ...
                            "frames": [
                                {
                                    ...
                                    "state": {
                                        ...
                                        "slots_values": {
                                            "slots_values_name": [
                                                sv_name
                                                for sv_name, sv_list in frame["state"][
                                                    "slot_values"
                                                ].items()
                                            ]
                                            if "state" in frame
                    ...
                    for turn in res["turns"]:
                        ...
                        if turn["speaker"] == "USER":
                            for name in self.ORDERED_TRACK_SLOTS:
                                ...
                                value = (
                                    frame["state"]["slots_values"]["slots_values_list"][
                                        frame["state"]["slots_values"][
                                            "slots_values_name"
                                        ].index(name)
                                    ]
                                    if name
                                    in frame["state"]["slots_values"][
                                        "slots_values_name"
                                    ]
                                    else "none"
                                )
                                ...
                                slot_res = {
                                    ...
                                    "service+description+history": f"{description} context: {history}",
                                    **deepcopy(res),
                                    **deepcopy(turn),
                                }
                                del slot_res["turns"]
