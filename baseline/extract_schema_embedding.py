...
from schema_guided_dst.baseline.bert import modeling, tokenization
...
def input_fn_builder(features, seq_length):
    ...
    def input_fn(params):
        ...
        tensors["embedding_tensor_name"] = tf.constant(
            all_features["embedding_tensor_name"], shape=[num_examples], dtype=tf.string
        )
...
class SchemaEmbeddingGenerator(object):
    ...
    def _get_goal_slots_and_values_input_features(self, service_schema):
        ...
        for slot_id, slot in enumerate(service_schema.non_categorical_slots):
            ...
            features.append(
                self._create_feature(
                    nl_seq, "noncat_slot_emb", service_schema.service_id, slot_id
                )
            )
        ...
        for slot_id, slot in enumerate(service_schema.categorical_slots):
            ...
            features.append(
                self._create_feature(
                    nl_seq, "cat_slot_emb", service_schema.service_id, slot_id
                )
            )
    ...
    def _get_input_fn(self, schemas):
        ...
        input_fn = input_fn_builder(features=features, seq_length=self._max_seq_length)
        return input_fn
            ...
            schema_embs.append(
                {
                    ...
                    "cat_slot_value_emb": np.zeros(
                        [max_num_cat_slot, max_num_value, embedding_dim]
                    ),
                }
            )
