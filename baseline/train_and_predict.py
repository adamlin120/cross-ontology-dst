...
from schema_guided_dst.baseline import (
    config,
    data_utils,
    extract_schema_embedding,
    pred_utils,
)
from schema_guided_dst.baseline.bert import modeling, optimization, tokenization
...
flags.DEFINE_string(
    "bert_ckpt_dir", None, "Directory containing pre-trained BERT checkpoint."
)
...
flags.DEFINE_float("dropout_rate", 0.1, "Dropout rate for BERT representations.")
...
flags.DEFINE_float(
    "num_train_epochs", 80.0, "Total number of training epochs to perform."
)
flags.DEFINE_float(
    "warmup_proportion",
    0.1,
...
flags.DEFINE_enum(
    "task_name", None, config.DATASET_CONFIG.keys(), "The name of the task to train."
)
...
flags.DEFINE_enum(
    "run_mode", None, ["train", "predict"], "The mode to run the script in."
)
...
def _file_based_input_fn_builder(
    dataset_config, input_dial_file, schema_embedding_file, is_training, drop_remainder
):
    ...
    name_to_features = {
        ...
        "service_id": tf.io.FixedLenFeature([], tf.int64),
        "utt": tf.io.FixedLenFeature([max_utt_len], tf.int64),
        "utt_mask": tf.io.FixedLenFeature([max_utt_len], tf.int64),
        "utt_seg": tf.io.FixedLenFeature([max_utt_len], tf.int64),
        "cat_slot_num": tf.io.FixedLenFeature([], tf.int64),
        "cat_slot_status": tf.io.FixedLenFeature([max_num_cat_slot], tf.int64),
        "cat_slot_value_num": tf.io.FixedLenFeature([max_num_cat_slot], tf.int64),
        "cat_slot_value": tf.io.FixedLenFeature([max_num_cat_slot], tf.int64),
        "noncat_slot_num": tf.io.FixedLenFeature([], tf.int64),
        "noncat_slot_status": tf.io.FixedLenFeature([max_num_noncat_slot], tf.int64),
        "noncat_slot_value_start": tf.io.FixedLenFeature(
            [max_num_noncat_slot], tf.int64
        ),
        "noncat_slot_value_end": tf.io.FixedLenFeature([max_num_noncat_slot], tf.int64),
        "noncat_alignment_start": tf.io.FixedLenFeature([max_utt_len], tf.int64),
        "noncat_alignment_end": tf.io.FixedLenFeature([max_utt_len], tf.int64),
        "req_slot_num": tf.io.FixedLenFeature([], tf.int64),
        "req_slot_status": tf.io.FixedLenFeature([max_num_total_slot], tf.int64),
        "intent_num": tf.io.FixedLenFeature([], tf.int64),
        "intent_status": tf.io.FixedLenFeature([max_num_intent], tf.int64),
    }
...
class SchemaGuidedDST(object):
    ...
    def define_model(self, features, is_training):
        ...
        self._encoded_utterance, self._encoded_tokens = self._encode_utterances(
    ...
    def define_loss(self, features, outputs):
        ...
        intent_loss = tf.losses.softmax_cross_entropy(
            onehot_intent_labels, intent_logits, weights=features["is_real_example"]
        )
        ...
        noncat_weights = tf.sequence_mask(
            features["noncat_slot_num"], maxlen=max_num_noncat_slots, dtype=tf.float32
        )
        ...
        onehot_end_labels = tf.one_hot(span_end_labels, max_num_tokens, dtype=tf.int32)
        # Zero out losses for non-categorical slot spans when the slot status is not
        # active.
        noncat_loss_weight = tf.cast(
            tf.equal(noncat_slot_status_labels, data_utils.STATUS_ACTIVE), tf.float32
        )
    ...
    def define_predictions(self, features, outputs):
        ...
        predictions["req_slot_status"] = tf.sigmoid(outputs["logit_req_slot_status"])
        ...
        total_scores = tf.expand_dims(start_scores, axis=3) + tf.expand_dims(
            end_scores, axis=2
        )
        ...
        invalid_index_mask = tf.tile(
            (start_idx > end_idx), [batch_size, max_num_slots, 1, 1]
        )
        total_scores = tf.where(
            invalid_index_mask, tf.zeros_like(total_scores), total_scores
        )
        max_span_index = tf.argmax(
            tf.reshape(total_scores, [-1, max_num_slots, max_num_tokens ** 2]), axis=-1
        )
    ...
    def _get_intents(self, features):
        ...
        repeated_null_intent_embedding = tf.tile(
            null_intent_embedding, [batch_size, 1, 1]
        )
        ...
        mask = tf.sequence_mask(features["intent_num"] + 1, maxlen=max_num_intents + 1)
    ...
    def _get_categorical_slot_goals(self, features):
        ...
        status_logits = self._get_logits(slot_embeddings, 3, "categorical_slot_status")
        ...
        value_logits = self._get_logits(
            value_embeddings_reshaped, 1, "categorical_slot_values"
        )
        ...
        # softmaxed.
        mask = tf.sequence_mask(features["cat_slot_value_num"], maxlen=max_num_values)
    ...
    def _get_noncategorical_slot_goals(self, features):
        ...
        status_logits = self._get_logits(
            slot_embeddings, 3, "noncategorical_slot_status"
        )
        ...
        layer_1 = tf.keras.layers.Dense(
            units=embedding_dim, activation=modeling.gelu, name="noncat_spans_layer_1"
        )
        ...
        tiled_token_mask = tf.tile(
            tf.expand_dims(tf.expand_dims(token_mask, 1), 3), [1, max_num_slots, 1, 2]
        )
...
def _create_dialog_examples(processor, dial_file):
    ...
    data_utils.file_based_convert_examples_to_features(
        frame_examples, processor.dataset_config, dial_file
    )
...
def main(_):
    ...
    dial_file_name = "{}_{}_examples.tf_record".format(task_name, FLAGS.dataset_split)
    ...
    if FLAGS.run_mode == "train":
        ...
        tf.compat.v1.logging.info(
            "  Num actual examples = %d", num_actual_predict_examples
        )
        ...
        for ckpt_num in ckpt_nums:
            tf.compat.v1.logging.info(
                "***** Predict results for %s set *****", FLAGS.dataset_split
            )

            predictions = estimator.predict(
                input_fn=predict_input_fn,
                checkpoint_path=os.path.join(
                    FLAGS.output_dir, "model.ckpt-%s" % ckpt_num
                ),
            )
            ...
            pred_utils.write_predictions_to_file(
                predictions, input_json_files, schema_json_file, prediction_dir
            )
