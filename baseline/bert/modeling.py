...
def dense_layer_3d(
    input_tensor, num_attention_heads, size_per_head, initializer, activation, name=None
):
...
def dense_layer_3d_proj(
    ...
    with tf.variable_scope(name):
        w = tf.get_variable(
            name="kernel", shape=[hidden_size, hidden_size], initializer=initializer
        )
...
def dense_layer_2d(input_tensor, output_size, initializer, activation, name=None):
