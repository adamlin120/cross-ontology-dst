...
class Dstc8DataProcessor(object):
    ...
    def get_dialog_examples(self, dataset):
        ...
        dialog_paths = [
            os.path.join(
                self.dstc8_data_dir, dataset, "dialogues_{:03d}.json".format(i)
            )
    ...
    def _create_examples_from_turn(
        ...
        base_example.add_utterance_features(
            system_tokens, system_inv_alignments, user_tokens, user_inv_alignments
        )
        examples = []
    ...
    def _find_subword_indices(
        self, slot_values, utterance, char_slot_spans, alignments, subwords, bias
    ):
...
class InputExample(object):
    ...
    def __init__(
        ...
        self.noncategorical_slot_status = [
            STATUS_OFF
    ...
    def add_utterance_features(
        self, system_tokens, system_inv_alignments, user_tokens, user_inv_alignments
    ):
    ...
    def add_noncategorical_slots(
        self, state_update, system_span_boundaries, user_span_boundaries
    ):
...
def file_based_convert_examples_to_features(dial_examples, dataset_config, output_file):
    ...
    for (ex_index, example) in enumerate(dial_examples):
        ...
        features["cat_slot_status"] = _create_int_feature(ex.categorical_slot_status)
...
def normalize_list_length(input_list, target_len, padding_unit):
    ...
    if len(input_list) < target_len:
        input_list.extend([padding_unit for _ in range(target_len - len(input_list))])
