...
class MetricsTest(absltest.TestCase):
    def setUp(self):
        ...
        schema_file = os.path.join(THIS_DIR, "test_data", "metrics_test_refschema.json")
    ...
    def test_active_intent_accuracy(self):
        ...
        intent_acc_hyp = metrics.get_active_intent_accuracy(
            self.frame_ref, self.frame_hyp
        )
        self.assertAlmostEqual(
            self.correct_metrics[metrics.ACTIVE_INTENT_ACCURACY], intent_acc_hyp
        )
    ...
    def test_slot_tagging_f1(self):
        ...
        # (2) Test on a previously known frame.
        slot_tagging_f1_hyp = metrics.get_slot_tagging_f1(
            self.frame_ref, self.frame_hyp, self.utterance, self.schema
        )
        self._assert_dicts_almost_equal(
            self.correct_metrics[metrics.SLOT_TAGGING_F1], slot_tagging_f1_hyp._asdict()
        )
    ...
    def test_average_and_joint_goal_accuracy(self):
        ...
        # Ground truth values for oracle prediction are all 1.0.
        self._assert_dicts_almost_equal(
            {k: 1.0 for k in ACCURACY_METRICS}, goal_accuracy_oracle_strict
        )
        ...
        self._assert_dicts_almost_equal(
            {k: self.correct_metrics[k] for k in ACCURACY_METRICS}, goal_accuracy_hyp
        )
